...
for step in range(n_steps):
    ...
    old_weight  = (rho_free(x[knext], x_old, dtau) *
                   ...
                   math.exp(-dtau * V_anharmonic(x_old, cubic, quartic)))
    new_weight  = (rho_free(x[knext], x_new, dtau) *
                   rho_free(x_new, x[kprev], dtau) *
                   math.exp(-dtau * V_anharmonic(x_new, cubic, quartic)))
...
pylab.title('$\pi(x)$ for quantum anharmonic oscilliator with $\\beta = 2^{%i}$, cubic=%s, quartic=%s' % (math.log(beta, 2), cubic, quartic))
...
pylab.savefig('plot-anharmonic-px_c2_beta%s_cubic_%s_quartic_%s.png' % (beta, cubic, quartic))
pylab.show()
