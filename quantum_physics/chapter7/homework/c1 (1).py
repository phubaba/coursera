...
#omega, name = [4.0, 4.0, 1.0], 'cigar' # example, please adapt # cigar
omega, name = [1.0, 5.0, 1.0], 'pancake' # example, please adapt # pancake
...
beta = 1.0 / T
print 'omega: ', omega
temp_set = 0.9 * (omega[0] * omega[1] * omega[2])**(1./3.) * N ** (1./3.)

print "Cycles predicted to set in for %s at %s. For T* of %s get a T of %s" % (name, temp_set, T_star, T)
# Initial condition
filename = 'data_boson_configuration_anisotropic_N_%s_T_Star_%s.txt' %(N, T_star)
positions = {}
...
pylab.title(str(N) + ' bosons at T* = ' + str(T_star) + "Shape: %s" % name)
...
#pylab.savefig('c1_plot_boson_configuration_N_%s_Tstar_%s_%s.png' % (N, T_star, name))
pylab.show()
